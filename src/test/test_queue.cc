...
struct F {
  ...
  explicit F(std::string str) : s(std::move(str)) {}
  F(const F &o) : s(o.s) {}
  F(F &&o) : s(std::move(o.s)) {}
...
class QueueTest : public ::testing::Test {
 protected:
  ...
  s21::queue<F> q3{F("hello"), F("world")};
};

bool operator==(const F &l, const F &r) { return r.s == l.s; }
...
TEST_F(QueueTest, Emplace) {
  ...
  for (int i = 0; i < 666; i++) {
    EXPECT_EQ(F.front(), B.front());
    EXPECT_EQ(F.back(), B.back());
    F.pop();
    B.pop();
  }
}
